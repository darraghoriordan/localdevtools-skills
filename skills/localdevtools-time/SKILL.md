---
name: localdevtools-time
description: Convert clipboard timestamps, explain cron schedules or compare an instant across timezones using local deterministic tools.
---
...
- `node scripts/run.mjs time convert --clipboard` accepts 10-digit Unix seconds or a supported ISO date.
- `node scripts/run.mjs cron explain --clipboard` or `cron next --clipboard`; add `--seconds` for six-field expressions. Results identify the system timezone used.
- `node scripts/run.mjs timezone compare --clipboard --zones 'Australia/Sydney,Europe/London'`; clipboard must be an ISO instant with Z or an explicit offset.
  Results are ordinary JSON. No scheduling task is created by these commands.
