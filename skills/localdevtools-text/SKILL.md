---
name: localdevtools-text
description: Transform clipboard text casing or lines locally. Use private output for confidential text and logs.
---
...
- `node scripts/run.mjs text case --clipboard --to snake` (also upper, lower, capital, pascal, camel, kebab, header, constant, title, sentence)
- `node scripts/run.mjs text lines --clipboard --sort asc --deduplicate`; options include `--locale`, `--prepend`, `--append`.


Text commands return ordinary results. Add `--private` for confidential text or logs.
