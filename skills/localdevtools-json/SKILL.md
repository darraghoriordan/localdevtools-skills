---
name: localdevtools-json
description: Validate, format or minify JSON on the clipboard locally. Use private output for confidential or production payloads.
---
...
- `node scripts/run.mjs json validate --clipboard`
- `node scripts/run.mjs json format --clipboard --indent 2 --sort-keys`
- `node scripts/run.mjs json minify --clipboard`
  These commands default to ordinary output. Add `--private` for sensitive data. Validation returns a boolean without the document. JSON is strict by default; `--tolerant` allows trailing commas and `--remove-comments` also permits comments. Use those options only when requested.
